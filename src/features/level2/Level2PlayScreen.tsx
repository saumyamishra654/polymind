...
import { level2Polygons } from '../../data/levelData'
...
import type { Level2Edge } from './level2Scoring'
import { scoreLevel2Round } from './level2Scoring'

type Level2PlayScreenProps = {
  onComplete: (results: Level2RoundResult[]) => void
}

type RoundPhase = 'memorize' | 'reconstruct'

export function Level2PlayScreen({ onComplete }: Level2PlayScreenProps) {
  const [roundIndex, setRoundIndex] = useState(0)
  const [results, setResults] = useState<Level2RoundResult[]>([])
  ...
  const {
    ...
    useReconstructionRound<{ viaHint: boolean }>()

  const vertices = level2Polygons[roundIndex].vertices
  ...
  const handleHint = () => {
    ...
    const candidates: HintCandidate<{ viaHint: boolean }>[] = Array.from(
  ...
  const handleSubmit = () => {
    ...
    const roundResult = scoreLevel2Round({
      vertices,
      edges: edges as Level2Edge[],
      edgeHistory: edgeHistory as Level2Edge[],
      hintsUsed,
      erasedEdges,
      timeTaken,
    ...
    if (roundIndex === level2Polygons.length - 1) {
        ...
        {phase === 'memorize' ? (
          ...
          <ReconstructionBoard
            ...
            onEndEdge={(vertexIndex) => {
              endDrag(vertexIndex, { viaHint: false })
            }}
